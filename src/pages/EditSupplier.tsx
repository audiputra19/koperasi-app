...
import { useGetSupplierQuery, useInputSupplierMutation } from "../services/apiSupplier";
...
const EditSupplier:FC = () => {
    const {id} = useParams();
    const [form, setForm] = useState({ kdSupp: id, nama: "", alamat: "" });
    const {data: dataSupp, isLoading: loadingSupp} = useGetSupplierQuery(undefined, {refetchOnMountOrArgChange: true});
    const supplier = dataSupp?.find(item => item.kode === id);
